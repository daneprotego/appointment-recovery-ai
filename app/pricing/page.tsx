...
const plans = [
  ...
  },
  {
    name: "Growth",
    price: "$39",
    description: "Recover more revenue with waitlist workflows.",
    features: [
      ...
      <section className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h1 className="mt-4 text-4xl font-bold tracking-tight md:text-6xl">
            Simple pricing for every appointment volume.
          </h1>

          <p className="mt-5 text-lg text-slate-600">
            Start free, then upgrade when automated SMS recovery and AI triage
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-4">
          {plans.map((plan) => (
            ...
            >
              {plan.highlighted && (
                <p className="mb-4 w-fit rounded-full bg-white/20 px-3 py-1 text-xs font-semibold">
                  Most popular
                </p>
              )}
              ...
              {plan.highlighted ? (
                <Link
                  href="/signup"
                  className="mt-8 block rounded-full border border-white bg-white px-5 py-3 text-center text-sm font-bold text-blue-700 transition hover:bg-blue-50"
                >
                  Start with Growth
                </Link>
              ) : (
